...
ssid = subprocess.check_output(
    ["nmcli -t -f active,ssid dev wifi | grep '^yes' | cut -d: -f2"], shell=True
).decode()
...
text_area = Label(
    terminalio.FONT, text=f"ssid:{ssid}\n{ip}", color=0xFFFFFF, line_spacing=0.5
)
uptime_grp = displayio.Group(scale=1, x=0, y=30)
uptime_txt_area = Label(terminalio.FONT, color=0xFF0000, text=" " * 20)
...
while True:
    ...
    uptime_txt_area.text = f"uptime: {round(time.monotonic(), 2)}"
    time.sleep(1 / 30)
